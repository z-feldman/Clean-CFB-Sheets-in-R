...
```{r}
all_teams <- map(.x = 1:length(sheets_temp),.f = ~tibble("team" = sheets_temp[[.x]]$team)) %>% bind_rows()
all_teams %>% unique()
```
...
We still need to fill in conference for everything other than preseason. Will also add week variable, this is just row_number integer divided by 130.
```{r}
...
full_data %<>% mutate(week = dplyr::row_number() %/% 130)
